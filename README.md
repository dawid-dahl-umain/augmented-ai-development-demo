...
# Run acceptance tests (15 tests)
pnpm test acceptance-test/

# Run isolation demo (9 tests)
pnpm test acceptance-test-isolation-demo/
...
-   Temporal isolation for rerunnable tests
...
```typescript
async executeMoves(moves: ReadonlyArray<string>): Promise<void> {
    const csv = moves.join(",")
    const args = ["--moves", csv]
    // Executes: node dist/index.js --moves 1,4,2,5,3
...
2. **Temporal isolation** - Rerunnable without cleanup
