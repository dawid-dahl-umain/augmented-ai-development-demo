...
# Run the demo CLI game
pnpm start -- --moves 1,4,2,5,3

# Launch the browser UI (opens at http://localhost:5173)
pnpm start:ui

# Run acceptance tests (defaults to CLI protocol)
pnpm test:at

# Run acceptance tests against the web protocol
...
- Protocol driver that executes the real system via CLI or web UI
...
│   ├── executable-specs/               # 16 BDD tests (maps 1:1 to specs)
...
**Executable Test** ([acceptance-test/executable-specs/](./acceptance-test/executable-specs/tictactoe.acceptance.spec.ts))
...
- Static HTML shell in `public/tictactoe.html`
- UI logic lives in `src/frontend/tictactoe-ui.ts` and imports the domain game (`src/domain/tic-tac-toe/game.ts`)
- Bundled alongside the CLI via `pnpm run build` (tsup emits ESM in `dist/`)
- Minimal Node `http` server in `src/adapters/web/server.ts` serves `public/` and the compiled bundle
- Run `pnpm start:ui`, then browse to <http://localhost:5173> to play
- Click board cells to place marks; use “Start New Game” to reset the match
...
frontend/
├── public/
│   └── tictactoe.html          # Single HTML entry point
├── src/frontend/
│   └── tictactoe-ui.ts         # DOM wiring + state management
└── src/adapters/web/
    └── server.ts               # Static asset server (build artefacts only)
```

### How It Fits the Architecture

- The browser UI is just another adapter on top of the same domain core. `tictactoe-ui.ts` calls `TicTacToe.start()` / `TicTacToe.play()` directly, so the immutable state machine in `src/domain/tic-tac-toe/` powers both the CLI and the HTML interface.
- No presenter/port changes were needed; the DOM handles rendering just like the CLI presenter renders text. The separation still mirrors the four-layer model (spec → DSL → driver → SUT), with the web driver targeting the DOM instead of stdout.
